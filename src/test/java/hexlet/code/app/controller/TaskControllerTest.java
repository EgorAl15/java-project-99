...
class TaskControllerTest {
  ...
  void testGetTasks() throws Exception {
    ...
    mockMvc
        .perform(get("/api/tasks").header("Authorization", "Bearer " + getToken()))
        .andExpect(status().isOk())
  ...
  void testGetTaskById() throws Exception {
    ...
    mockMvc
        .perform(get("/api/tasks/" + task.getId()).header("Authorization", "Bearer " + getToken()))
        .andExpect(status().isOk())
  ...
  void testDeleteTask() throws Exception {
    var task = createTask("Test title", "Test content", draftStatus, Set.of(feature));

    mockMvc
        .perform(
            delete("/api/tasks/" + task.getId()).header("Authorization", "Bearer " + getToken()))
  ...
  void testCannotDeleteAssignedUser() throws Exception {
    ...
    mockMvc
        .perform(
            delete("/api/users/" + user.getId()).header("Authorization", "Bearer " + getToken()))
  ...
  void testCannotDeleteUsedStatus() throws Exception {
    ...
    mockMvc
        .perform(
            delete("/api/task_statuses/" + draftStatus.getId())
                .header("Authorization", "Bearer " + getToken()))
        .andExpect(status().isConflict());
  }
  ...
  void testCannotDeleteUsedLabel() throws Exception {
    ...
    mockMvc
        .perform(
            delete("/api/labels/" + feature.getId())
                .header("Authorization", "Bearer " + getToken()))
        .andExpect(status().isConflict());
  }
  ...
  void testFilterByStatus() throws Exception {
    ...
    mockMvc
        ...
        .andExpect(status().isOk())
        .andExpect(header().string("X-Total-Count", "1"))
        .andExpect(jsonPath("$[0].title").value("Fix application bug"))
  ...
  void testFilterByLabel() throws Exception {
    ...
    mockMvc
        .perform(
            get("/api/tasks")
                .param("labelId", bug.getId().toString())
                .header("Authorization", "Bearer " + getToken()))
        .andExpect(status().isOk())
        .andExpect(header().string("X-Total-Count", "1"))
        .andExpect(jsonPath("$[0].title").value("Fix application bug"));
  ...
  private void createFilterTasks() {
    createTask("Create new version", "First task", draftStatus, Set.of(feature));

    createTask("Fix application bug", "Second task", reviewStatus, Set.of(bug));
  }

  private Task createTask(String title, String content, TaskStatus taskStatus, Set<Label> labels) {
